...
if ((obj->type == OBJ_ROBOT) && (obj->id == SPECIAL_REACTOR_ROBOT))
   ...
   switch(obj->type) {
      ...
      case OBJ_POWERUP:
         obj->control_type = CT_POWERUP;
         nd_read_byte((sbyte *)&(obj->movement_type));   // might have physics
                                                         //  movement
...
int newdemo_read_frame_information()
{
   ...
   while( !done ) {
      ...
      case ND_EVENT_VIEWER_OBJECT:           // Followed by an object structure
         nd_read_byte((sbyte *)&WhichWindow);
         if (WhichWindow&15)
           {
...
//          do_controlcen_destroyed_stuff(NULL);
      ...
      case ND_EVENT_CLOAKING_WALL: {
         ...
         nd_read_byte((sbyte *)&type);
         nd_read_byte((sbyte *)&state);
         nd_read_byte((sbyte *)&cloak_value);
         nd_read_short(&l0);
         nd_read_short(&l1);
         nd_read_short(&l2);
         nd_read_short(&l3);
         ...
         if (JustStartedPlayback)
          {
            ...
            for (i=0;i<Num_walls;i++)    // restore the walls
               {
               nd_read_byte((sbyte *)&Walls[i].type);
               nd_read_byte((sbyte *)&Walls[i].flags);
               nd_read_byte((sbyte *)&Walls[i].state);
