...
multi_do_release_robot(char *buf)
{
	...
	botnum = objnum_remote_to_local( INTEL_SHORT( *(short *)(buf+2) ), (sbyte)buf[4] );
...
multi_do_robot_position(char *buf)
{
	...
	botnum = objnum_remote_to_local( INTEL_SHORT( *(short *)(buf+loc) ), (sbyte)buf[loc+2] );
																loc += 3;
...
multi_do_robot_explode(char *buf)
{
	...
	pnum = buf[loc]; 					loc += 1;
	killer = objnum_remote_to_local( INTEL_SHORT( *(short *)(buf+loc) ), (sbyte)buf[loc+2]);
											loc += 3;
	botnum = objnum_remote_to_local( INTEL_SHORT( *(short *)(buf+loc) ), (sbyte)buf[loc+2]);
											loc += 3;
   thief=buf[loc];
