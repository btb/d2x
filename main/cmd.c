...
const char *cmd_complete(char *input)
{
	...
	for (i = 0; i < Num_cmds; i++)
		if (!strnicmp(input, cmd_list[i]->name, len))
			return cmd_list[i]->name;

	for (i = 0; i < Num_cmd_aliases; i++)
		if (!strnicmp(input, cmd_alias_list[i]->name, len))
...
void cmd_free(void)
{
	...
	hashtable_free(&cmd_hash);
	hashtable_free(&cmd_alias_hash);
}


void cmd_init(void)
{
	hashtable_init(&cmd_hash, CMD_MAX_CMDS);
	hashtable_init(&cmd_alias_hash, CMD_MAX_ALIASES);
