...
void wall_open_door(segment *seg, int side)
{
	...
	if (w->state == WALL_DOOR_CLOSING) {		//closing, so reuse door
		...
		for (i=0;i<Num_open_doors;i++) {		//find door
			...
			if (d->front_wallnum[0] == WALL_NUMBER(w) ||
				d->back_wallnum[0] == WALL_NUMBER(w) ||
				(d->n_parts == 2 && (d->front_wallnum[1] == WALL_NUMBER(w) ||
				                     d->back_wallnum[1] == WALL_NUMBER(w))))
				break;
		}
