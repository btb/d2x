	...
	if (fate == HIT_WALL) {
		...
		if (wall_num != -1) {
			...
			for (i=0;i<Num_open_doors;i++) {		//find door
				d = &ActiveDoors[i];
				if (d->front_wallnum[0] == WALL_NUMBER(wall) || d->back_wallnum[0] == WALL_NUMBER(wall) ||
					(d->n_parts == 2 && (d->front_wallnum[1] == WALL_NUMBER(wall) || d->back_wallnum[1] == WALL_NUMBER(wall))))
					break;
			} 
...
static void gamecntl_cmd_EnableCheats(int argc, char **argv)
{
	if (Game_mode & GM_MULTI)
		return;
...
extern int Ugly_robot_cheat, Ugly_robot_texture;
...
static void gamecntl_cmd_JohnCheat4(int argc, char **argv)
{
	...
	Ugly_robot_texture = atoi(argv[1]);
